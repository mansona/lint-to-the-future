...
module('Acceptance | basic', function (hooks) {
  ...
  test('basic functionality', async function (assert) {
    ...
    assert
      .dom(
        '[data-test-chart="lint-to-the-future-ember-template:require-valid-alt-text"] [data-test-time-button="monthly"]',
      )
      .isChecked();
    assert
      .dom(
        '[data-test-chart="lint-to-the-future-eslint:ember/no-empty-glimmer-component-classes"] [data-test-time-button="daily"]',
      )
      .isChecked();
    assert
      .dom(
        '[data-test-chart="lint-to-the-future-eslint:prettier/prettier"] [data-test-time-button="weekly"]',
      )
      .isChecked();
  ...
  test('completed rules are filtered into collapsed window', async function (assert) {
    ...
    assert
      .dom('[data-test-completed-rules] [data-test-chart]')
      .exists({ count: 2 });
  });
});
